...
const ListPasivaNeracaNormal = ({ data, levelAkun = [], totalSemuaAktiva }) => {
  let totalSemua = [];
  let totalUtang = [];
  return (
    <>
      ...
      {data?.map((d) => {
        let total = [];
        // let nilaiKategori = d?.total;
        // if (d?.nama == "EKUITAS") {
        //   nilaiKategori =
        //     d?.total - totalSemuaAktiva.reduce((a, b) => a + b, 0);
        // }
        // total.push(nilaiKategori);
        // console.log(d);
        ...
        return (
          <>
            ...
            {d?.akunNeraca?.map((e) => {
              ...
              //   nilaiAkun =
              //     totalSemuaAktiva.reduce((a, b) => a + b, 0) -
              //     e?.total -
              //     totalUtang.reduce((a, b) => a + b, 0);
              // }
              // if (d?.nama.includes("UTANG")) {
              //   totalUtang.push(nilaiAkun);
              // }
              // totalSemua.push(nilaiAkun);
              // total.push(nilaiAkun);
              return (
                <tr>
...
export default ListPasivaNeracaNormal;
