...
const ListJurnalUmum = ({ data, setEditData, _deleteMutasi, isRencana }) => {
    ...
    <>
      <tr>
        <td className="border-start-0" rowSpan={jurnal?.length}>
          {momentPackage(data?.tanggal).format(
            isRencana ? "MMMM YYYY" : "DD/MM/YYYY"
          )}
        </td>
