...
import { checkFormJadwal } from "utilities/PPDBUtils";
...
const ModalJadwalUjianPenerimaan = ({
  editData,
  _getJadwalUjian,
  isEdit,
  jalur,
  ...
  useEffect(() => {
    setFormData({
      ...
      // keterangan: window.$(`#keterangan-ujian`).summernote("code", editData?.keterangan),
    });
    setCurrent(0);
  }, [editData]);
  ...
  // useEffect(() => getUjianData(), []);
  useEffect(() => {
    getUjianData();
  }, []);

  // useEffect(() => {
  //   setFormData({ ...formData, m_gelombang_ppdb_id: "" });
  // }, [formData?.m_jalur_id]);
  ...
  const _postJadwalPPDB = async () => {
    ...
    const payload = {
      ...
      m_gelombang_ppdb_id: formData?.m_gelombang_ppdb_id,
    };

    // payload.keterangan = window.$(`#keterangan-ujian`).summernote("code");
    ...
    if (data) {
      ...
      // window.$(`#keterangan-ujian`).summernote("reset");
      hideModal("ModalJadwalUjianPenerimaan");
      _getJadwalUjian();
    } else {
  ...
  const next = () => {
    const check = checkFormJadwal(formData, current);
    if (check) {
      toast.error(check);
      return;
    }
  ...
  const currentJalur = jalur?.find((d) => d?.id == formData?.m_jalur_id);
      ...
      <div className="row mb-4">
        ...
        <div className="col-md-6">
          ...
          <SelectShared
            ...
            value={formData.m_gelombang_ppdb_id}
            options={jalur
              ?.find((d) => d?.id == formData?.m_jalur_id)
              ?.gelombang?.map((d) => {
                return {
                  label: d?.nama,
                  value: d?.id,
          ...
          <div className="row my-4 gy-4">
            <div className="col-md-6">
              ...
              <DatePicker
                ...
                //   current <
                //     momentPackage(currentJalur?.dibuka).startOf("day") ||
                //   current > momentPackage(currentJalur?.ditutup).endOf("day")
                // }
  ...
  return (
    <>
      <ModalStep
        modalId="ModalJadwalUjianPenerimaan"
...
export default ModalJadwalUjianPenerimaan;
