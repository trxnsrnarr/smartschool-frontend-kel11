...
const ModalTindakLanjutKerjaGuru = () => {
  const { bukuKerjaDetailData, refetchBukuKerjaDetail, refetchData } =
    useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTindakLanjutKerjaGuru) ||
    null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTindakLanjutKerjaGuru");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTindakLanjutKerjaGuru", null);
      }
    }
  };
      ...
      title={
        <>
          ...
          <span className="fs-6 fw-normal">
            ...
            } program tindak kanjut kerja guru`}
          </span>
        </>
      }
      content={
        <>
          ...
          <div className="form-label">Lampiran Tindak Lanjut Guru</div>
          <CompleteFileUpload
...
export default ModalTindakLanjutKerjaGuru;
