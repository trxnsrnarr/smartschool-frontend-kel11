...
const ModalTambahKumpulanKisiSoal = () => {
  const { bukuKerjaDetailData, refetchBukuKerjaDetail, refetchData } =
    useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahKumpulanKisiSoal) ||
    null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahKumpulanKisiSoal");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahKumpulanKisiSoal", null);
      }
    }
  };
    ...
    <NewModal
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} Kumpulan Kisi Soal`}
          </h4>
      ...
      content={
        <>
          ...
          <div className="form-label">Lampiran Kumpulan Kisi Soal</div>
          <CompleteFileUpload
...
export default ModalTambahKumpulanKisiSoal;
