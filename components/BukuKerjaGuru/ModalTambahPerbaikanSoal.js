...
const ModalTambahPerbaikanSoal = () => {
  const { bukuKerjaDetailData, refetchBukuKerjaDetail, refetchData } =
    useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahPerbaikanSoal) || null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahPerbaikanSoal");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahPerbaikanSoal", null);
      }
    }
  };
    ...
    <NewModal
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} Perbaikan Soal`}
          </h4>
      ...
      content={
        <>
          ...
          <div className="form-label">Lampiran Perbaikan Soal</div>
          <CompleteFileUpload
...
export default ModalTambahPerbaikanSoal;
