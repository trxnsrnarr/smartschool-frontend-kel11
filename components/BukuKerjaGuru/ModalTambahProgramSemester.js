...
const ModalTambahProgramSemester = () => {
  const { bukuKerjaDetailData, refetchBukuKerjaDetail, refetchData } =
    useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahProgramSemester) ||
    null;
  ...
  const validateData = () => {
    ...
    if (!formData?.kkm) {
      toast.error("Anda belum memasukkan kkm");
      return false;
    }
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahProgramSemester");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahProgramSemester", null);
      }
    }
  };
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} Program Semester`}
          </h4>
        ...
        <>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              autoComplete="off"
              placeholder="Tuliskan kkm"
              value={formData?.kkm}
              type="number"
              onChange={(e) => changeFormData("kkm", e.target.value)}
            />
          </div>
          <div className="form-label">Lampiran Alokasi Waktu</div>
          <CompleteFileUpload
...
export default ModalTambahProgramSemester;
