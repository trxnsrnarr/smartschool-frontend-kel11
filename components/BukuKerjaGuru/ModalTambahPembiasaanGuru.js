...
const ModalTambahPembiasaanGuru = () => {
  const { refetchBukuKerjaDetail, refetchData } = useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahPembiasaanGuru) || null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahPembiasaanGuru");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahPembiasaanGuru", null);
      }
    }
  };
  ...
  return (
    <NewModal
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} Pembiasaan Guru`}
          </h4>
      ...
      content={
        <>
          <div className="form-label">Lampiran Pembiasaan Guru</div>
          <CompleteFileUpload
...
export default ModalTambahPembiasaanGuru;
