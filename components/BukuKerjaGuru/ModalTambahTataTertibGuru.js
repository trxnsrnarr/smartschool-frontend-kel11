...
const ModalTambahTataTertibGuru = () => {
  const { refetchBukuKerjaDetail, refetchData } = useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahTataTertibGuru) || null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahTataTertibGuru");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahTataTertibGuru", null);
      }
    }
  };
  ...
  return (
    <NewModal
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} Tata Tertib Guru`}
          </h4>
      ...
      content={
        <>
          <div className="form-label">Lampiran Tata Tertib Guru</div>
          <CompleteFileUpload
...
export default ModalTambahTataTertibGuru;
