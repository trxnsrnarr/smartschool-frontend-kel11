...
const ModalTambahJurnalAgendaGuru = () => {
  const { bukuKerjaDetailData, refetchBukuKerjaDetail, refetchData } =
    useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahJurnalAgendaGuru) ||
    null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahJurnalAgendaGuru");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahJurnalAgendaGuru", null);
      }
    }
  };
    ...
    <NewModal
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} Jurnal Agenda Guru`}
          </h4>
      ...
      content={
        <>
          ...
          <div className="form-label">Lampiran Jurnal Guru</div>
          <CompleteFileUpload
...
export default ModalTambahJurnalAgendaGuru;
