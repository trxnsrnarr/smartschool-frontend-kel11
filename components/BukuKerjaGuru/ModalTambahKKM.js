...
const ModalTambahKKM = () => {
  const { bukuKerjaDetailData, refetchBukuKerjaDetail, refetchData } =
    useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahKKM) || null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahKKM");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahKKM", null);
      }
    }
  };
    ...
    <NewModal
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} KKM / KKTP`}
          </h4>
      ...
      content={
        <>
          ...
          <div className="form-label">Lampiran KKM / KKTP</div>
          <CompleteFileUpload
