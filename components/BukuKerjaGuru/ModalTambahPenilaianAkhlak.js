...
const ModalTambahPenilaianAkhlak = () => {
  const { bukuKerjaDetailData, refetchBukuKerjaDetail, refetchData } =
    useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahPenilaianAkhlak) ||
    null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahPenilaianAkhlak");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahPenilaianAkhlak", null);
      }
    }
  };
    ...
    <NewModal
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} Penilaian Akhlak`}
          </h4>
      ...
      content={
        <>
          ...
          <div className="form-label">Lampiran Penilaian Akhlak</div>
          <CompleteFileUpload
...
export default ModalTambahPenilaianAkhlak;
