...
const ModalTambahSklKiKd = () => {
  const { bukuKerjaDetailData, refetchBukuKerjaDetail, refetchData } =
    useBukuKerjaDetail();
  const editModalData =
    useEditModal((state) => state.editModal?.modalTambahSklKiKd) || null;
  ...
  const handleSubmit = async () => {
    if (validateData()) {
      ...
      if (data) {
        hideModal("modalTambahSklKiKd");
        refetchBukuKerjaDetail(!refetchData);
        setFormData(initialFormData);
        setEditModal("modalTambahSklKiKd", null);
      }
      setButtonState("idle");
    }
  };
    ...
    <NewModal
      ...
      title={
        <>
          <h4 className="mb-1 fw-extrabold">
            {`${editModalData ? "Ubah" : "Tambah"} SKL, KI / CP dan KD / TP`}
          </h4>
      ...
      content={
        <>
          ...
          <div className="form-label">Lampiran SKL, KI / CP, dan KD / TP</div>
          <CompleteFileUpload
...
export default ModalTambahSklKiKd;
