...
const SoalIsian = ({
  ...
  // const [jawaban, setJawaban] = useState(soalSiswa?.jawabanIsian);
  ...
  const onClickSave = () => {
    const jawaban = window.$(`#editorJawabanIsian`).summernote("code");
    handlePutJawabanSiswa({ jawabanIsian: jawaban });
  };

  // useEffect(() => {
  //   setJawaban(soalSiswa?.jawabanIsian || "");
  // }, [soalSiswa?.jawabanIsian]);

  useEffect(() => {
    // Dikomen karena kalo pake ini ntah kenapa pas upload file ga nge hit endpoint firebase
    // window.$(`#editorJawabanIsian`).summernote("reset");
    // window.$(`#editorJawabanIsian`).summernote("code", soalSiswa?.jawabanIsian);
  }, [soalSiswa]);
        ...
        <div className="col-md-12 mb-5">
          <div className="card card-ss p-4 mb-5">
            ...
            <div className="mb-4">
              ...
              /> */}
              <Editor
                id="editorJawabanIsian"
                defaultValue={soalSiswa?.jawabanIsian}
                ujian
              />
              {/* <input
                type="file"
