...
const PilihJurusanPPDB = () => {
  ...
  const _getGelombangPPDB = async () => {
    ...
    if (data) {
      ...
      setFormData({ ...formData, ...data?.gelombangAktif });
      setTerdaftar(data);
      setPembayaran(JSON.parse(data?.gelombangAktif?.pembayaran || "[]"));
    }
  };
    ...
    <Layout>
      <AnimatePage>
        <div className="container my-5">
                ...
                <div className="card-body p-4">
                      ...
                      <table className="w-100">
                            ...
                            <span className="color-dark fw-bold">
                              {
                                bankData?.find(
                                  (d) =>
                                    d.value == gelombangAktif?.gelombang?.bank
                                )?.label
                              }
                            </span>
                          </th>
                        ...
                        <tr>
                          ...
                          <th>
                            :{" "}
                            <span className="color-dark fw-bold">
                              {gelombangAktif?.gelombang?.norek}
                            </span>
                          </th>
                        </tr>
                        <tr>
                          ...
                          <th>
                            ...
                            <span className="color-dark fw-bold">
                              {" "}
                              {gelombangAktif?.gelombang?.namaAkun}{" "}
                            </span>{" "}
                          </th>
                        </tr>
                        <tr>
                  ...
                  <div className="bg-soft-primary text-center color-dark py-2 my-4 rounded-ss border border-primary-ss">
                    ...
                    {gelombangAktif?.diskon?.diskon ? (
                      <>
                        <div
                          className="m-0 fw-black color-dark"
                          style={{ textDecoration: "line-through" }}
                        >
                          {currencyFormatter(
                            gelombangAktif?.gelombang?.biayaPendaftaran ||
                            gelombangAktif?.gelombang?.jalur?.biaya
                          )}
                        </div>
                        <h2 className="m-0 fw-black color-primary">
                          {currencyFormatter(gelombangAktif?.diskon?.harga)}
                        </h2>
                      </>
                    ) : (
                      <h2 className="m-0 fw-black color-primary">
          ...
          {/* {gelombangAktif?.nominal ? (
            <CardRiwayatTransaksi
              riwayat={gelombangAktif}
              isTerkonfirmasi={gelombangAktif?.diverifikasi}
            />
          ) : null} */}
        </div>
