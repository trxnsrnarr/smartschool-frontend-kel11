...
import ModalTambahAktivitas from "components/Keuangan/ModalTambahAktivitas";
...
const index = ({ subnav }) => {
  ...
  const [akunTerpakai, setAkunTerpakai] = useState([]);
  ...
  const _postKategori = async () => {
    ...
    const { data } = await postKategoriArus({
      ...formData,
    });
  ...
  const _getArus = async () => {
    ...
    if (data) {
      ...
      data?.kategori?.map((d) => {
        d?.tipeAkun?.map((e) => {
          akunTerpakai.push(e.mKeuKategoriTipeAkunId);
        });
      });
      setAkunTerpakai([...akunTerpakai]);
  ...
  const _postAktivitas = async () => {
    ...
    if (!formData.mKeuKategoriTipeAkunId) {
  ...
  const _putAktivitas = async (id, form) => {
    ...
    if (!payload.mKeuKategoriTipeAkunId) {
      toast.error("Harap memilih tipe akun");
      return;
    }
    setFormData({ ...formData, btnBio: "loading" });
  ...
  const _postRumusKenaikan = async () => {
    ...
    setFormData({ ...formData, btnBio: "loading" });
    const { data } = await postRumusKenaikan({
  ...
  const _postRumusKasAwal = async () => {
    ...
    setFormData({ ...formData, btnBio: "loading" });
    const { data } = await postRumusKasAwal({
      rumus: JSON.stringify(currentRumus),
    });
  ...
  const _postRumusKasAkhir = async () => {
    ...
    setFormData({ ...formData, btnBio: "loading" });
    const { data } = await postRumusKasAkhir({
      rumus: JSON.stringify(currentRumus),
    });
  ...
  return (
    ...
    >
      <AnimatePage>
        <HeaderLaporanArusKas ssURL={ssURL} />
        <div className="row">
              ...
              <div className="mb-4">
                ...
                {rumusKenaikan ? (
                  ...
                  <a
                    ...
                    className="btn-tambah-rumus-laba-rugi d-flex align-items-center"
                      ...
                      {JSON.parse(rumusAwal?.rumus)
                        .map((d) => {
                          if (d.operator) {
                            ...
                            return keuangan.find((e) => e.id == d.id)?.nama;
                          }
                        })
                        .join(" ")}
                    </h6>
