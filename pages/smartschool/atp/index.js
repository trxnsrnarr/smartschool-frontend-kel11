import ModalTambahATP from "components/RPP/ModalTambahATP";
...
const index = ({ subnav }) => {
  ...
  const _postSilabus = async () => {
    if (!formData.judul) {
      toast.error("Isi Judul Alur Tujuan Pembelajaran");
      return;
    }
    if (!formData.lampiran) {
      toast.error("Lampirkan file Alur Tujuan Pembelajaran");
      return;
    }
    // if (!formData.mMataPelajaranId) {
    //   toast.error("Pilih Mata Pelajaran ATP");
    //   return;
    // }
    setFormData({ ...formData, buttonState: "loading" });
    const { data } = await postRpp({ ...formData, tipe: "atp" });
  ...
  const _getSilabus = async () => {
    const { data } = await getRpp({ tipe: "atp" });
