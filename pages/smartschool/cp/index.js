...
const index = ({ subnav }) => {
  ...
  const _postSilabus = async () => {
    if (!formData.judul) {
      toast.error("Isi Judul Capaian Pembelajaran");
      return;
    }
    if (!formData.lampiran) {
      toast.error("Lampirkan file Capaian Pembelajaran");
      return;
    }
    ...
    const { data } = await postRpp({ ...formData, tipe: "cp" });
  ...
  const _getSilabus = async () => {
    const { data } = await getRpp({ tipe: "cp" });
