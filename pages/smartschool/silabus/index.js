...
import ModalTambahSilabus from "components/RPP/ModalTambahSilabus";
...
const index = ({ subnav }) => {
  ...
  const _postSilabus = async () => {
    if (!formData.judul) {
      toast.error("Isi Judul Silabus");
      return;
    }
    if (!formData.lampiran) {
      toast.error("Lampirkan file silabus");
      return;
    }
    if (!formData.mMataPelajaranId) {
      toast.error("Pilih Mata Pelajaran Silabus");
      return;
    }
    setFormData({ ...formData, buttonState: "loading" });
    const { data } = await postRpp({ ...formData, tipe: "silabus" });
  ...
  const _getSilabus = async () => {
    const { data } = await getRpp({ tipe: "silabus" });
