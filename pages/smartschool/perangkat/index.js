...
import ModalTambahPerangkat from "components/RPP/ModalTambahPerangkat";
...
const index = ({ subnav }) => {
  ...
  const _postPerangkat = async () => {
    if (!formData.judul) {
      toast.error("Isi Judul Perangkat");
      return;
    }
    if (!formData.lampiran) {
      toast.error("Lampirkan file Perangkat");
      return;
    }
    if (!formData.mMataPelajaranId) {
      toast.error("Pilih Mata Pelajaran Perangkat");
      return;
    }
    setFormData({ ...formData, buttonState: "loading" });
    const { data } = await postRpp({ ...formData, tipe: "perangkat" });
  ...
  const _getPerangkat = async () => {
    const { data } = await getRpp({ tipe: "perangkat" });
  ...
  const _deletePerangkat = async (id) => {
    swal({
    ...
    <Layout>
      <AnimatePage>
        ...
        <ModalTambahPerangkat
          _editPerangkat={_editPerangkat}
          _postPerangkat={_postPerangkat}
          formData={formData}
