...
const index = (props) => {
  ...
  const onClickEdit = (editData) => {
    showModal("modalTambahAnalisisButirSoal");
    setEditModal("modalTambahAnalisisButirSoal", editData);
  };
  return (
      ...
      <div className="row mt-4 pt-4 g-4">
        {datas?.map((data) => (
          <CardBukuKerja
            ...
            bottomLabel={[
              { icon: "/img/icon-mapel.svg", text: data?.mataPelajaran?.nama },
              {
                icon: "/img/icon-tanggal-2.svg",
                text: momentPackage(data?.tanggal).format("dddd, MMMM YYYY"),
              },
            ]}
