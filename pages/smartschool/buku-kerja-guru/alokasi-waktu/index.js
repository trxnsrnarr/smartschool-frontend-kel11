...
const index = (props) => {
  ...
  const onClickEdit = (editData) => {
    showModal("modalTambahAlokasiWaktu");
    setEditModal("modalTambahAlokasiWaktu", editData);
  };

  return (
    <LayoutBukuKerjaDetail>
      <Title>Alokasi Waktu</Title>
      ...
      <div className="row mt-4 pt-4 g-4">
        {datas?.map((data) => (
          <CardBukuKerja
            ...
            bottomLabel={[
              {
                icon: "/img/icon-jam.svg",
                text: `${data?.waktu} Jam Pelajaran`,
              },
            ]}
