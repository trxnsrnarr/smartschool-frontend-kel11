      ...
      <div className="row mt-4 pt-4 g-4">
        { datas?.map(data =>
          <CardBukuKerja
            ...
            bottomLabel={[ { icon: "/img/icon-mapel.svg", text: data?.mataPelajaran?.nama } ]}
            backgroundIcon="/img/icon-buku-kerja-3.png"
            onClick={() => router.push(`${ssURL}/buku-kerja-guru/daftar-kehadiran/${data?.id}?rombel_id=${data?.rombel?.id}&mata_pelajaran_id=${data?.mataPelajaran?.id}&nav=rekap`)}
          />
        )}
      </div>
    </LayoutBukuKerjaDetail>
