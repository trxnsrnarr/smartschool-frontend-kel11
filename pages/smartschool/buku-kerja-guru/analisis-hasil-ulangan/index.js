...
const index = (props) => {
  ...
  const onClickEdit = (editData) => {
    showModal("modalTambahAnalisisHasilUlangan");
    setEditModal("modalTambahAnalisisHasilUlangan", editData);
  }
  return (
    <LayoutBukuKerjaDetail>
      ...
      <div className="row mt-4 pt-4 g-4">
        { datas?.map(data =>
          <CardBukuKerja
            key={data?.id}
            title={data?.rombel?.nama}
            topLabel={[{ text: `${data?.ta?.tahun} - ${data?.ta?.semester}` }]}
            bottomLabel={[ { icon: "/img/icon-mapel.svg", text: data?.mataPelajaran?.nama }, { icon: "/img/icon-tanggal-2.svg", text: momentPackage(data?.tanggal).format("dddd, MMMM YYYY") } ]}
