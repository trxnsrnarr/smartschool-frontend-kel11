...
const index = (props) => {
  ...
  const onClickEdit = (editData) => {
    showModal("modalEvaluasiDiriKerjaGuru");
    setEditModal("modalEvaluasiDiriKerjaGuru", editData);
  }
    ...
    <LayoutBukuKerjaDetail>
      ...
      <div className="row mt-4 pt-4 g-4">
        { datas?.map(data =>
          <CardBukuKerja
            key={data?.id}
            title={`${data?.ta?.tahun} - ${data?.ta?.semester}`}
            bottomLabel={[ { icon: "/img/icon-skl-ki-kd.svg", text: momentPackage(data?.tanggal).format("dddd, DD MMM YYYY") } ]}
            backgroundIcon="/img/icon-buku-kerja-4.png"
