...
import { deleteTopik, getDetailTopik, postTopik } from "client/TopikClient";
...
const index = ({ id, topik_id, kelasId }) => {
  topik_id = parseInt(topik_id);
  ...
  const handlePostTopikData = async (formData) => {
    const { data } = await postTopik(formData);
    if (data) {
      getDetailBabData();
      router.push(
        `${ssURL}/kelas/${kelasId}/materi/${data.topikId}?babId=${id}`
      );
    } else {
    ...
    swal({
      ...
      if (willDelete) {
        ...
        if (data) {
          ...
          router.push(`${ssURL}/kelas/${kelasId}/materi/-1?babId=${id}`);
        } else {
          setButtonState("error");
        }
      }
  ...
  return (
    <AnimatePage>
      <LayoutDetail
        title={bab?.judul}
        backProps={`${ssURL}/kelas/${kelasId}/materi`}
        modalWrapper={
          <>
            <NewModal
