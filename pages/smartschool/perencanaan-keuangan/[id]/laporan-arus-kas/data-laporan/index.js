...
const index = ({
  ...
  const _getArus = async () => {
    const { data, error } = await getArusLaporan(id, {
      tanggalAwal1,
      tanggalAkhir1,
      tanggalAwal2,
      tanggalAkhir2,
    ...
    if (data) {
      ...
      console.log({ kumulatifData, kumulatifData2 });
      ...
      const { data: ArusKasAjak, error } = await getArusLaporanNeraca(id, {
  ...
  useEffect(() => {
    router.push({
      pathname: `/smartschool/perencanaan-keuangan/${id}/laporan-arus-kas/data-laporan`,
      query: {
        ...router.query,
