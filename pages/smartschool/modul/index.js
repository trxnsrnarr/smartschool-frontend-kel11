import ModalTambahModul from "components/RPP/ModalTambahModul";
...
const index = ({ subnav }) => {
  ...
  const _postSilabus = async () => {
    if (!formData.judul) {
      toast.error("Isi Judul modul");
      return;
    }
    if (!formData.lampiran) {
      toast.error("Lampirkan file modul");
      return;
    }
    // if (!formData.mMataPelajaranId) {
    //   toast.error("Pilih Mata Pelajaran modul");
    //   return;
    // }
    setFormData({ ...formData, buttonState: "loading" });
    const { data } = await postRpp({ ...formData, tipe: "modul" });
  ...
  const _getSilabus = async () => {
    const { data } = await getRpp({ tipe: "modul" });
